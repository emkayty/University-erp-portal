...
type OutstandingCourse = Pick<
  ...
  'courseId' | 'code' | 'title' | 'creditUnits' | 'level' | 'semester' | 'isCompulsory'
>;
...
export class AcademicService {
  ...
  async getJourney(studentId: string) {
    const student = await this.prisma.student.findUniqueOrThrow({
      ...
      include: {
        ...
        courseRegs: {
          include: { courseOffering: { include: { course: true, semesterModel: true } } },
        ...
        results: {
          ...
          orderBy: [{ semester: { academicYear: 'asc' } }, { createdAt: 'asc' }],
        },
    ...
    const outstanding = this.outstandingForJourney(degreeAudit?.policySnapshot, degreeAudit?.requirementResults, student.results);
    const outstandingRequirementGroups = this.unmetGroupsForJourney(degreeAudit?.policySnapshot, degreeAudit?.requirementResults);
    ...
    return {
      ...
      progress: {
        ...
        percent: required ? Math.min(100, Math.round((credits / required) * 100)) : 0,
  ...
  async getLatestDegreeAuditForUser(userId: string) {
    const student = await this.prisma.student.findUniqueOrThrow({ where: { userId }, select: { id: true, curriculumVersionId: true } });
  ...
  async runDegreeAudit(studentId: string, actorId: string) {
    const result = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      ...
      const exemptions = await tx.academicExemption.findMany({
        where: { studentId, status: APPROVED, approvedById: { not: null }, approvedAt: { not: null } },
        select: { id: true, curriculumRequirementId: true },
      });
      const substitutions = await tx.academicSubstitution.findMany({
        where: { studentId, status: APPROVED, approvedById: { not: null }, approvedAt: { not: null } },
        select: { id: true, curriculumRequirementId: true, substituteCourseId: true },
      });
      ...
      const equivalencies = await tx.courseEquivalency.findMany({
        ...
        select: { id: true, fromCourseId: true, toCourseId: true, direction: true },
      });

      const { requirementGroups, catalog } = this.buildRequirementCatalog(groups, programmeCourses, student.curriculumVersionId, student.programme.minCreditUnits);
      const requirementIds = new Set(catalog.map((entry) => entry.curriculumRequirementId));
      const curriculumCourseIds = new Set(catalog.map((entry) => entry.courseId));
      const applicableExemptions = exemptions.filter((record) => requirementIds.has(record.curriculumRequirementId));
      const applicableSubstitutions = substitutions.filter((record) => requirementIds.has(record.curriculumRequirementId) && curriculumCourseIds.has(record.substituteCourseId));
      const applicableTransfers = transfers.filter((record) => !record.mappedCourseId || curriculumCourseIds.has(record.mappedCourseId));
      const applicableEquivalencies = equivalencies.filter((record) => curriculumCourseIds.has(record.fromCourseId) || curriculumCourseIds.has(record.toCourseId));
      ...
      const auditInput: DegreeAuditInput = {
        attempts,
        exemptions: applicableExemptions.map((record): ExemptionInput => ({ curriculumRequirementId: record.curriculumRequirementId })),
        substitutions: applicableSubstitutions.map((record): SubstitutionInput => ({
        ...
        equivalencies: applicableEquivalencies.map((record): EquivalencyInput => ({
        ...
        graduationRequirements: [
          {
            ...
            config: { minCredits: student.programme.minCreditUnits },
            isMandatory: true,
          },
          {
        ...
        totalElapsedYears: Math.max(0, new Date().getFullYear() - Number(student.entryAcademicYear.slice(0, 4))),
      };
      ...
      const audit = await tx.degreeAudit.create({
        data: {
          ...
          requirementResults: engineResult as unknown as Prisma.InputJsonValue,
  ...
  async runProgression(studentId: string, actorId: string) {
    const result = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      ...
      if (!latest) {
        throw new BadRequestException('No academic period record exists for progression evaluation.');
      }
      const now = new Date();
      ...
      const selectPolicy = (policyType: string) => {
        ...
        if (!record) throw new BadRequestException(`Selected ${policyType} policy record cannot be loaded.`);
        return record;
      };
      ...
      const progressionRule = this.parseProgressionRule(progressionPolicy.ruleDefinition);
      const standingRule = this.parseStandingRule(standingPolicy.ruleDefinition);
      ...
      const priorStandings: PriorStandingInput[] = priorStandingRows
        .filter((record) => {
          ...
          return ['GOOD_STANDING', 'WARNING', 'PROBATION', SUSPENSION_RECOMMENDED].includes(record.standing);
        })
        .map((record) => ({
      ...
      if (existingEvaluation && existingStanding) {
        ...
        if (!placement) {
          const decision = existingStanding.standing === SUSPENSION_RECOMMENDED
            ? 'SUSPEND'
          ...
          placement = await tx.academicPlacement.create({
            data: {
              ...
              toLevel: decision === 'PROMOTE' || decision === 'PROMOTE_WITH_CARRYOVER' ? student.level + 100 : student.level,
              decision,
              status: 'RECOMMENDED',
              reason: 'Placement backfilled from an existing progression and standing decision.',
              policySnapshot: {
        ...
        return { progression: existingEvaluation, standing: existingStanding, placement };
      }

      const progression = evaluateProgression(period, progressionRule);
      const standing = evaluateAcademicStanding(period, priorStandings, standingRule);
      ...
      const placementDecision = standing.standing === SUSPENSION_RECOMMENDED
        ? 'SUSPEND'
      ...
      const placement = await tx.academicPlacement.create({
        data: {
          ...
          academicYear: latest.academicYear,
          fromLevel: student.level,
          toLevel: placementDecision === 'PROMOTE' || placementDecision === 'PROMOTE_WITH_CARRYOVER' ? student.level + 100 : student.level,
  ...
  async applyPlacement(placementId: string, actorId: string) {
    const applied = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const placement = await tx.academicPlacement.findUniqueOrThrow({ where: { id: placementId } });
      ...
      if (!['RECOMMENDED', 'APPROVED'].includes(placement.status)) {
        throw new ConflictException(`Academic placement is ${placement.status} and cannot be applied.`);
      }
      const student = await tx.student.findUniqueOrThrow({ where: { id: placement.studentId } });
      ...
      switch (placement.decision) {
        ...
        case 'PROMOTE_WITH_CARRYOVER':
          ...
          break;
        default:
          throw new BadRequestException(`Unsupported academic placement decision: ${placement.decision}`);
      }
    ...
    await this.audit.log(
      {
        ...
        targetId: applied.id,
        newValues: { status: applied.status, decision: applied.decision, appliedById: actorId },
      },
      actorId,
    );
    return applied;
  ...
  async submitAppealForUser(userId: string, dto: SubmitAcademicAppealDto) {
    ...
    await this.audit.log(
      { action: AuditAction.CREATE, targetTable: 'academic_appeals', targetId: appeal.id, newValues: { studentId: student.id, appealType: appeal.appealType } },
      userId,
    );
    return appeal;
  }

  async decideAppeal(appealId: string, dto: DecideAcademicAppealDto, actorId: string) {
    const appeal = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const current = await tx.academicAppeal.findUniqueOrThrow({ where: { id: appealId } });
      if (current.status !== 'SUBMITTED') {
        throw new ConflictException(`Academic appeal is ${current.status} and cannot be decided again.`);
      }
      return tx.academicAppeal.update({
        where: { id: current.id },
        data: { status: dto.decision, decision: dto.rationale.trim(), decidedById: actorId, decidedAt: new Date() },
      });
    });
    await this.audit.log(
      { action: AuditAction.UPDATE, targetTable: 'academic_appeals', targetId: appeal.id, newValues: { status: appeal.status, decidedById: actorId } },
      actorId,
    );
    return appeal;
  }

  async requestProgrammeTransferForUser(userId: string, dto: RequestProgrammeTransferDto) {
    const student = await this.prisma.student.findUniqueOrThrow({ where: { userId }, select: { id: true, programmeId: true } });
    if (student.programmeId === dto.toProgrammeId) {
      throw new BadRequestException('The requested programme is already the student\'s current programme.');
    }
    const request = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`programme-transfer:${student.id}`}))`;
      await tx.programme.findUniqueOrThrow({ where: { id: dto.toProgrammeId }, select: { id: true } });
      const open = await tx.programmeTransferRequest.findFirst({
        where: { studentId: student.id, status: { in: ['SUBMITTED', 'UNDER_REVIEW'] } },
        select: { id: true },
      });
      if (open) throw new ConflictException('An open programme transfer request already exists for this student.');
    ...
    await this.audit.log(
      { action: AuditAction.CREATE, targetTable: 'programme_transfer_requests', targetId: request.id, newValues: { studentId: student.id, toProgrammeId: dto.toProgrammeId } },
      userId,
    );
    return request;
  }

  async decideProgrammeTransfer(requestId: string, dto: DecideProgrammeTransferDto, actorId: string) {
    const request = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const current = await tx.programmeTransferRequest.findUniqueOrThrow({ where: { id: requestId } });
      ...
      if (!['SUBMITTED', 'UNDER_REVIEW'].includes(current.status)) {
        throw new ConflictException(`Programme transfer request is ${current.status} and cannot be decided again.`);
      }
      if (dto.decision === 'APPROVED') {
        const currentStudent = await tx.student.findUniqueOrThrow({ where: { id: current.studentId }, select: { programmeId: true } });
        if (currentStudent.programmeId !== current.fromProgrammeId) {
          throw new ConflictException('The student programme changed after this transfer request was submitted. Re-open the request before approving it.');
        }
        ...
        if (!targetCurriculum) {
          throw new UnprocessableEntityException({
            ...
            message: 'The target programme has no active curriculum version; transfer cannot be applied.',
          });
        }
        await tx.student.update({
          where: { id: current.studentId },
          data: { programmeId: targetProgramme.id, departmentId: targetProgramme.departmentId, curriculumVersionId: targetCurriculum.id },
        });
      }
    ...
    await this.audit.log(
      { action: AuditAction.UPDATE, targetTable: 'programme_transfer_requests', targetId: request.id, newValues: { status: request.status, mappedCredits: request.mappedCredits } },
      actorId,
    );
    return request;
  }

  async requestInterruptionForUser(userId: string, dto: RequestAcademicInterruptionDto) {
    const startDate = new Date(dto.startDate);
    const endDate = dto.endDate ? new Date(dto.endDate) : null;
    if (endDate && endDate < startDate) throw new BadRequestException('Interruption end date cannot precede its start date.');
    const student = await this.prisma.student.findUniqueOrThrow({ where: { userId }, select: { id: true } });
    const interruption = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      ...
      if (open) throw new ConflictException('An active academic interruption request already exists for this student.');
      return tx.academicInterruption.create({
        data: { studentId: student.id, type: dto.type.trim().toUpperCase(), startDate, endDate, reason: dto.reason?.trim(), status: 'REQUESTED' },
      });
    });
    await this.audit.log(
      { action: AuditAction.CREATE, targetTable: 'academic_interruptions', targetId: interruption.id, newValues: { studentId: student.id, type: interruption.type } },
      userId,
    );
    return interruption;
  }

  async decideInterruption(interruptionId: string, dto: DecideAcademicInterruptionDto, actorId: string) {
    const interruption = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const current = await tx.academicInterruption.findUniqueOrThrow({ where: { id: interruptionId } });
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`academic-interruption:${current.studentId}`}))`;
      if (current.status !== 'REQUESTED') {
        throw new ConflictException(`Academic interruption is ${current.status} and cannot be decided again.`);
      }
      if (dto.decision === 'APPROVED') {
        await tx.student.update({ where: { id: current.studentId }, data: { status: StudentStatus.DEFERRED } });
      }
      return tx.academicInterruption.update({
        where: { id: current.id },
        data: { status: dto.decision, decidedById: actorId, decidedAt: new Date() },
      });
    });
    await this.audit.log(
      { action: AuditAction.UPDATE, targetTable: 'academic_interruptions', targetId: interruption.id, newValues: { status: interruption.status } },
      actorId,
    );
    return interruption;
  }
  ...
  async resumeInterruption(interruptionId: string, actorId: string) {
    const interruption = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      ...
      if (current.status !== 'APPROVED') {
        throw new ConflictException(`Academic interruption is ${current.status} and cannot be resumed.`);
      }
      const now = new Date();
      if (current.endDate && current.endDate > now) {
        throw new ConflictException('This academic interruption has not reached its end date.');
      }
      const student = await tx.student.findUniqueOrThrow({ where: { id: current.studentId }, select: { status: true } });
      if (student.status === StudentStatus.DEFERRED) {
        await tx.student.update({ where: { id: current.studentId }, data: { status: StudentStatus.ACTIVE } });
      }
      return tx.academicInterruption.update({
        where: { id: current.id },
        data: { status: 'COMPLETED', endDate: current.endDate ?? now, decidedById: actorId, decidedAt: now },
      });
    });
    await this.audit.log(
      { action: AuditAction.UPDATE, targetTable: 'academic_interruptions', targetId: interruption.id, newValues: { status: interruption.status, resumedById: actorId } },
      actorId,
    );
    return interruption;
  }
  ...
  async issueCredential(dto: IssueAcademicCredentialDto, actorId: string) {
    const credential = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const student = await tx.student.findUniqueOrThrow({ where: { id: dto.studentId }, select: { id: true, status: true, matricNo: true, programmeId: true } });
      if (student.status !== StudentStatus.GRADUATED) {
        throw new UnprocessableEntityException({
          code: 'CREDENTIAL_STUDENT_NOT_GRADUATED',
          message: 'Academic credentials can only be issued for a graduated student.',
        });
      }
      return tx.academicCredential.create({
        data: {
          ...
          status: 'ISSUED',
          snapshot: { ...dto.snapshot, studentId: student.id, matricNo: student.matricNo, programmeId: student.programmeId, issuedById: actorId } as unknown as Prisma.InputJsonValue,
        },
      });
    });
    await this.audit.log(
      { action: AuditAction.CREATE, targetTable: 'academic_credentials', targetId: credential.id, newValues: { studentId: credential.studentId, credentialType: credential.credentialType, credentialNo: credential.credentialNo } },
      actorId,
    );
    return credential;
  }

  async revokeCredential(credentialId: string, reason: string, actorId: string) {
    const credential = await this.prisma.runExclusive(this.rlsContext, async (tx) => {
      const current = await tx.academicCredential.findUniqueOrThrow({ where: { id: credentialId } });
      if (current.status !== 'ISSUED') {
        throw new ConflictException(`Academic credential is ${current.status} and cannot be revoked.`);
      }
      return tx.academicCredential.update({
        ...
        data: {
          status: 'REVOKED',
          revokedAt: new Date(),
          snapshot: { ...(current.snapshot as Record<string, unknown>), revocationReason: reason.trim(), revokedById: actorId } as Prisma.InputJsonValue,
        },
      });
    });
    await this.audit.log(
      { action: AuditAction.UPDATE, targetTable: 'academic_credentials', targetId: credential.id, newValues: { status: credential.status, revocationReason: reason.trim() } },
      actorId,
    );
    return credential;
  }
  ...
  private async latestAudit(studentId: string, curriculumVersionId?: string) {
    return this.prisma.degreeAudit.findFirst({
      where: { studentId, ...(curriculumVersionId ? { curriculumVersionId } : {}) },
      orderBy: { createdAt: 'desc' },
    });
  }
  ...
  private async latestPlan(studentId: string, curriculumVersionId?: string) {
    return this.prisma.academicPlan.findFirst({
      where: { studentId, status: ACTIVE, ...(curriculumVersionId ? { curriculumVersionId } : {}) },
      include: { items: { orderBy: { sequence: 'asc' }, include: { course: true } } },
      orderBy: { createdAt: 'desc' },
    });
  }
  ...
  private async replaceAcademicPlan(
    ...
    unmetRequirementGroups: Array<{ requirementGroupId: string; unmetReasons: string[]; unmetRequirementIds: string[]; needsReview: boolean }>,
  ) {
    ...
    return tx.academicPlan.create({
      ...
      include: { items: { include: { course: true }, orderBy: { sequence: 'asc' } } },
    });
  }
  ...
  ): { requirementGroups: RequirementGroupInput[]; catalog: RequirementCatalogEntry[] } {
    if (groups.length) {
      ...
      const requirementGroups = groups.map((group) => ({
        ...
        requirements: group.requirements.map((requirement) => {
          if (requirement.courseId && requirement.course) {
            const matchingProgrammeCourse = programmeCourses.find((course) => course.courseId === requirement.courseId);
            catalog.push({
  ...
  private outstandingFromEngine(
    result: { requirementGroupResults: Array<{ unmetReasons: string[]; unmetRequirementIds?: string[] }> },
    catalog: RequirementCatalogEntry[],
  ): OutstandingCourse[] {
    const unmetRequirementIds = new Set(
      result.requirementGroupResults.flatMap((group) => group.unmetRequirementIds ?? []),
    );
    ...
    if (!unmetRequirementIds.size) {
      for (const reason of result.requirementGroupResults.flatMap((group) => group.unmetReasons)) {
        const match = /^Compulsory requirement not satisfied: course (.+)$/.exec(reason);
        if (match?.[1]) {
          const fallback = catalog.find((course) => course.courseId === match[1]);
          if (fallback) unmetRequirementIds.add(fallback.curriculumRequirementId);
        }
      }
    }
    return catalog
      .filter((course) => unmetRequirementIds.has(course.curriculumRequirementId))
      .sort((a, b) =>
        a.level - b.level ||
      ...
      .map(({ courseId, code, title, creditUnits, level, semester, isCompulsory }) => ({
        courseId,
        code,
        title,
        creditUnits,
  ...
  private outstandingForJourney(
    ...
    results: Array<{ courseOffering: { courseId: string; course: { code: string; title: string; creditUnits: number } }; grade: string }>,
  ) {
    const snapshot = policySnapshot as { requirementCatalog?: RequirementCatalogEntry[] } | null;
    const engine = requirementResults as { requirementGroupResults?: Array<{ unmetReasons?: string[]; unmetRequirementIds?: string[] }> } | null;
    if (snapshot?.requirementCatalog && engine?.requirementGroupResults) {
      return this.outstandingFromEngine(
        {
          requirementGroupResults: engine.requirementGroupResults.map((group) => ({
            unmetReasons: Array.isArray(group.unmetReasons) ? group.unmetReasons : [],
            unmetRequirementIds: Array.isArray(group.unmetRequirementIds) ? group.unmetRequirementIds : [],
          })),
        },
    ...
    const passed = new Set(results.filter((record) => record.grade !== 'F').map((record) => record.courseOffering.courseId));
    const failed = new Map<string, { courseId: string; code: string; title: string; credits: number }>();
  ...
  private unmetGroupsForJourney(
    policySnapshot: unknown,
    requirementResults: unknown,
  ): Array<{ requirementGroupId: string; unmetReasons: string[]; unmetRequirementIds: string[]; needsReview: boolean }> {
    const snapshot = policySnapshot as { unmetRequirementGroups?: Array<{ requirementGroupId?: string; unmetReasons?: string[]; unmetRequirementIds?: string[]; needsReview?: boolean }> } | null;
    ...
    const engine = requirementResults as { requirementGroupResults?: Array<{ requirementGroupId?: string; unmetReasons?: string[]; unmetRequirementIds?: string[]; needsReview?: boolean }> } | null;
    return (engine?.requirementGroupResults ?? [])
      .filter((group) => (group.unmetReasons?.length ?? 0) > 0 || group.needsReview)
      .map((group) => ({
        requirementGroupId: group.requirementGroupId ?? 'unknown',
        unmetReasons: Array.isArray(group.unmetReasons) ? group.unmetReasons : [],
        unmetRequirementIds: Array.isArray(group.unmetRequirementIds) ? group.unmetRequirementIds : [],
  ...
  private parseProgressionRule(ruleDefinition: unknown): ProgressionPolicyRule {
    const rule = this.requireRuleObject(ruleDefinition, PROGRESSION_POLICY_TYPE);
    ...
    if (
      ...
      throw new UnprocessableEntityException({
        ...
        message: 'The selected progression policy has an invalid rule definition.',
      });
    }
    return {
      ...
      maxCarryoversForConditionalProgress: rule.maxCarryoversForConditionalProgress,
  ...
  private parseStandingRule(ruleDefinition: unknown): StandingPolicyRule {
    ...
    if (
      ...
      throw new UnprocessableEntityException({
        code: 'ACADEMIC_POLICY_INVALID',
        message: 'The selected academic-standing policy has an invalid rule definition.',
      });
    }
    return {
      probationCgpaThreshold: rule.probationCgpaThreshold,
      warningCgpaThreshold: rule.warningCgpaThreshold,
      consecutiveProbationPeriodsForSuspension: rule.consecutiveProbationPeriodsForSuspension,
    };
  }

  private requireRuleObject(value: unknown, policyType: string): RuleDefinition {
