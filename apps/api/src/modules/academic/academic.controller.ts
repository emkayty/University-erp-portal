...
export class AcademicController {
  ...
  @Get("graduation-policies")
  @Roles("SUPER_ADMIN", "VC", "REGISTRAR", "DEAN")
  @ApiOperation({
    summary: "List governed graduation-requirement policy versions",
  })
  listGraduationPolicies(
    @Query("scope") scope?: CreateGraduationPolicyDto["scope"],
    @Query("scopeId") scopeId?: string,
  ) {
    return this.academic.listGraduationPolicies(scope, scopeId);
  }

  @Post("graduation-policies")
  @Roles("SUPER_ADMIN", "REGISTRAR", "DEAN")
  @ApiOperation({
    summary: "Create a draft graduation-requirement policy version",
  })
  createGraduationPolicy(
    @Body() dto: CreateGraduationPolicyDto,
    @CurrentUser() user: JwtPayload,
  ) {
    return this.academic.createGraduationPolicy(dto, user.sub);
  }

  @Post("graduation-policies/:policyId/activate")
  @Roles("SUPER_ADMIN", "VC", "REGISTRAR")
  @ApiOperation({
    summary: "Activate a validated graduation-requirement policy version",
  })
  activateGraduationPolicy(
    @Param("policyId") policyId: string,
    @CurrentUser() user: JwtPayload,
  ) {
    return this.academic.activateGraduationPolicy(policyId, user.sub);
  }
  ...
  @ApiOperation({
    summary: "Get a student academic journey for authorized academic officers",
  })
  ...
  @Roles("STUDENT")
  @ApiOperation({
    summary: "Get the latest degree audit for the authenticated student",
  })
  ...
  @ApiOperation({
    summary:
      "Evaluate and persist policy-driven progression and academic standing",
  })
  runProgression(
    @Param("studentId") studentId: string,
    @CurrentUser() user: JwtPayload,
  ...
  @ApiOperation({
    summary:
      "Apply an approved academic placement to the operational student record",
  })
  applyPlacement(
  ...
  @Roles("STUDENT")
  @ApiOperation({
    summary: "Submit an academic appeal with optional evidence reference",
  })
  submitAppeal(
    @CurrentUser() user: JwtPayload,
    @Body() dto: SubmitAcademicAppealDto,
  ) {
  ...
  requestProgrammeTransfer(
    @CurrentUser() user: JwtPayload,
    @Body() dto: RequestProgrammeTransferDto,
  ) {
  ...
  @ApiOperation({
    summary:
      "Approve or reject a programme transfer and atomically apply an approved target curriculum",
  })
  ...
  requestInterruption(
    @CurrentUser() user: JwtPayload,
    @Body() dto: RequestAcademicInterruptionDto,
  ) {
  ...
  @ApiOperation({
    summary:
      "Resume a completed academic interruption when its end date has elapsed",
  })
  resumeInterruption(
    @Param("interruptionId") interruptionId: string,
    @CurrentUser() user: JwtPayload,
  ...
  @Roles("SUPER_ADMIN", "REGISTRAR")
  @ApiOperation({
    summary: "Issue an immutable academic credential to a graduated student",
  })
  issueCredential(
    @Body() dto: IssueAcademicCredentialDto,
    @CurrentUser() user: JwtPayload,
  ) {
  ...
  @Roles("SUPER_ADMIN", "REGISTRAR")
  @ApiOperation({
    summary: "Revoke an issued academic credential with an auditable reason",
  })
  revokeCredential(
