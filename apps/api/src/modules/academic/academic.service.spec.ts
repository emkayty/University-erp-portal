...
import { CreateGraduationPolicyDto } from "./dto/academic-lifecycle.dto";
...
function createService() {
  const tx = {
    ...
    programme: { findUniqueOrThrow: jest.fn(), findMany: jest.fn() },
...
describe("AcademicService lifecycle safeguards", () => {
  ...
  it("loads approved exemptions, substitutions, transfers and effective equivalencies into the persisted audit evidence", async () => {
    ...
    tx.programmeCourse.findMany.mockResolvedValue([
      {
        ...
        semester: "FIRST",
        isCompulsory: true,
        course: { code: "GST101", title: "Communication", creditUnits: 2 },
      },
      {
        courseId: "course-2",
        level: 100,
        semester: "SECOND",
        isCompulsory: true,
        course: { code: "GST102", title: "Academic Writing", creditUnits: 2 },
      },
    ]);
    ...
    tx.academicExemption.findMany.mockResolvedValue([
      {
        id: "exemption-1",
        curriculumRequirementId: "requirement-1",
        approvedById: "officer-1",
        approvedAt: NOW,
      },
    ]);
    tx.academicSubstitution.findMany.mockResolvedValue([
      {
        id: "substitution-1",
        curriculumRequirementId: "requirement-2",
        substituteCourseId: "course-2",
        approvedById: "officer-1",
        approvedAt: NOW,
      },
    ]);
    ...
    tx.courseEquivalency.findMany.mockResolvedValue([
      {
        id: "equivalency-1",
        fromCourseId: "course-4",
        toCourseId: "course-1",
        direction: "BIDIRECTIONAL",
      },
    ]);
    tx.degreeAudit.create.mockImplementation(
    ...
    expect(tx.academicPlan.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { studentId: "student-1", status: "ACTIVE" },
        data: { status: "SUPERSEDED" },
      }),
    );
    expect(saved.policySnapshot).toEqual(
      expect.objectContaining({
        exceptionSourceIds: {
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ targetTable: "degree_audits" }),
      "officer-1",
    );
  });
  ...
  it("preserves unmet elective requirement IDs in the canonical plan input", async () => {
        ...
        results: [
          {
            ...
            courseOffering: {
              courseId: "course-1",
              course: { code: "ELEC101", title: "Elective One" },
            },
            creditUnits: 3,
            grade: "A",
            attemptNumber: 1,
    ...
    tx.programmeCourse.findMany.mockResolvedValue([
      {
        ...
        semester: "FIRST",
        isCompulsory: false,
        course: { code: "ELEC101", title: "Elective One", creditUnits: 3 },
      },
      {
        courseId: "course-2",
        level: 100,
        semester: "SECOND",
        isCompulsory: false,
        course: { code: "ELEC102", title: "Elective Two", creditUnits: 3 },
      },
    ]);
    tx.academicRequirementGroup.findMany.mockResolvedValue([
      {
        ...
        groupType: "DEPARTMENTAL_ELECTIVE",
        minCourses: 2,
        maxCourses: null,
        minCreditUnits: null,
        maxCreditUnits: null,
    ...
    tx.academicPlan.updateMany.mockResolvedValue({ count: 0 });
    tx.academicPlan.create.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "plan-elective",
        ...data,
        items: [],
    ...
    expect(
      (
        audit.policySnapshot as {
          unmetRequirementGroups: Array<{ unmetRequirementIds: string[] }>;
        }
      ).unmetRequirementGroups[0]?.unmetRequirementIds,
    ).toEqual(["req-2"]);
    expect(tx.academicPlan.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          items: {
            create: [expect.objectContaining({ courseId: "course-2" })],
          },
        }),
      }),
    );
  ...
  it("chooses the programme-scoped active policies and supplies prior standing history to the standing engine", async () => {
    ...
    tx.academicPolicyVersion.findMany.mockResolvedValue([
      {
        ...
        ruleDefinition: {
          ...
          minCgpaForUnconditionalProgress: 2,
      ...
      },
      {
        ...
        ruleDefinition: {
          minCreditUnitsToProgress: 18,
          minCgpaForUnconditionalProgress: 2.4,
          maxCarryoversForConditionalProgress: 1,
          conditionalProgressionAction: "REPEAT_PLACEMENT",
        },
      },
      {
        id: "standing-programme",
        ...
        approvalStatus: "ACTIVE",
        ruleDefinition: {
          probationCgpaThreshold: 1,
          warningCgpaThreshold: 2.7,
          consecutiveProbationPeriodsForSuspension: 2,
        },
      },
    ]);
    tx.academicStanding.findMany.mockResolvedValue([
      {
        academicHistoryId: "history-0",
        standing: "PROBATION",
        academicHistory: { startDate: new Date("2025-01-01") },
      },
    ]);
    tx.progressionEvaluation.findUnique.mockResolvedValue(null);
    tx.academicStanding.findUnique.mockResolvedValue(null);
    tx.progressionEvaluation.create.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "progression-1",
        ...data,
      }),
    );
    tx.academicStanding.create.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "standing-1",
        ...data,
      }),
    );
    tx.academicPlacement.create.mockResolvedValue({
      id: "placement-1",
      status: "RECOMMENDED",
    });
    ...
    expect(tx.academicStanding.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          policySnapshot: expect.objectContaining({ priorStandingCount: 1 }),
        }),
      }),
    );
  });

  it("creates a structured graduation-policy draft with explicit effective dates", async () => {
    const { service, tx, audit } = createService();
    tx.academicPolicyVersion.create.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "policy-1",
        ...data,
      }),
    ...
    const draft = await service.createGraduationPolicy(
      {
        scope: "PROGRAMME",
        scopeId: "programme-1",
        priority: 5,
        effectiveFrom: "2026-08-15T00:00:00.000Z",
        effectiveTo: "2027-08-15T00:00:00.000Z",
        ruleDefinition: {
          requirements: [
            {
              graduationRequirementId: "credits",
              requirementType: "MIN_CREDITS",
              config: { minCredits: 120 },
              isMandatory: true,
            },
            {
              graduationRequirementId: "cgpa",
      ...
      } as CreateGraduationPolicyDto,
      "registrar-1",
    );

    expect(draft.id).toBe("policy-1");
    expect(tx.academicPolicyVersion.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          approvalStatus: "DRAFT",
          scope: "PROGRAMME",
          scopeId: "programme-1",
          ruleDefinition: expect.objectContaining({
            requirements: expect.arrayContaining([
              expect.objectContaining({ requirementType: "MIN_CREDITS" }),
              expect.objectContaining({ requirementType: "MIN_CGPA" }),
            ]),
          }),
        }),
      }),
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ targetTable: "academic_policy_versions" }),
      "registrar-1",
    );
  });

  it("activates only a valid draft and records approval provenance while expiring a prior active version", async () => {
    const { service, tx, audit } = createService();
    tx.academicPolicyVersion.findUniqueOrThrow.mockResolvedValue({
      id: "policy-2",
      policyType: "GRADUATION_REQUIREMENTS",
      scope: "PROGRAMME",
      scopeId: "programme-1",
      approvalStatus: "DRAFT",
      effectiveTo: null,
    ...
    tx.programme.findMany.mockResolvedValue([{ minCreditUnits: 120 }]);
    tx.academicPolicyVersion.updateMany.mockResolvedValue({ count: 1 });
    tx.academicPolicyVersion.update.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "policy-2",
        approvalStatus: "ACTIVE",
        ...data,
    ...
    const activated = await service.activateGraduationPolicy(
      "policy-2",
      "senate-secretary-1",
    );

    expect(tx.academicPolicyVersion.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: { approvalStatus: "EXPIRED", effectiveTo: expect.any(Date) },
      }),
    );
    expect(tx.academicPolicyVersion.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: {
          approvalStatus: "ACTIVE",
          approvedById: "senate-secretary-1",
          approvedAt: expect.any(Date),
        },
      }),
    );
    expect(activated.approvalStatus).toBe("ACTIVE");
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("resumes an approved interruption only after its end date and restores a deferred student", async () => {
    ...
    tx.academicInterruption.update.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "interruption-1",
        ...data,
      }),
    ...
    const resumed = await service.resumeInterruption(
      "interruption-1",
      "registrar-1",
    );
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ targetTable: "academic_interruptions" }),
      "registrar-1",
    );
  });
  ...
  it("applies an independently authorized suspension placement to the operational student record", async () => {
    const { service, tx, audit } = createService();
    tx.academicPlacement.findUniqueOrThrow.mockResolvedValue({
      id: "placement-1",
      studentId: "student-1",
      status: "RECOMMENDED",
      decision: "SUSPEND",
    ...
    tx.academicPlacement.update.mockImplementation(
      ({ data }: { data: Record<string, unknown> }) => ({
        id: "placement-1",
        decision: "SUSPEND",
        ...data,
    ...
    const placement = await service.applyPlacement(
      "placement-1",
      "registrar-1",
    );
    ...
    expect(tx.student.update).toHaveBeenCalledWith({
      where: { id: "student-1" },
      data: { level: 200, status: StudentStatus.SUSPENDED },
    });
    expect(placement.status).toBe("APPLIED");
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ targetTable: "academic_placements" }),
      "registrar-1",
    );
  });
});
