...
import {
  ...
  AcademicActorRole,
} from "../../common/authorization/academic-offering-authorization.service";
...
export class AssessmentService {
  ...
  async findAccessibleOfferings(
    actorId: string,
    actorRole: AcademicActorRole = "STAFF",
  ) {
    ...
    if (actorRole === "STAFF") where.lecturer = { userId: actorId };
      ...
      where.course = { department: { hod: { userId: actorId } } };
    else if (actorRole === "DEAN")
      where.course = { department: { faculty: { dean: { userId: actorId } } } };
    ...
    return this.prisma.courseOffering.findMany({
      ...
      select: {
        ...
        semesterModel: {
          select: { name: true, academicYear: true, semesterNumber: true },
        },
        lecturer: { select: { firstName: true, lastName: true } },
      },
      orderBy: [
        { semesterModel: { academicYear: "desc" } },
        { course: { code: "asc" } },
        { sectionCode: "asc" },
      ],
    });
  ...
  async createScheme(
    ...
    await this.offeringAuthorization.assertOfferingAccess(
    ...
    if (!offering?.semesterId)
      throw new BadRequestException(
        "Course offering must be linked to a semester",
      );
    ...
    await this.audit.log(
      {
        ...
        targetTable: "assessment_schemes",
        targetId: scheme.id,
        newValues: { courseOfferingId: dto.courseOfferingId, version },
      },
      actorId,
    );
    return scheme;
  ...
  async setComponents(
    ...
    const schemeScope = await this.prisma.assessmentScheme.findUnique({
      where: { id: schemeId },
      select: { courseOfferingId: true },
    });
    if (!schemeScope)
      throw new BadRequestException("Assessment scheme was not found");
    await this.offeringAuthorization.assertOfferingAccess(
      schemeScope.courseOfferingId,
      actorId,
      actorRole,
    );
    ...
    if (Math.abs(total - 100) > 0.001)
      throw new BadRequestException(
        `Assessment component weights must total 100%; received ${total}`,
      );
    if (components.some((c) => c.maxScore <= 0))
      throw new BadRequestException(
        "Every assessment component must have a positive maximum score",
      );
    return this.prisma.$transaction(async (tx) => {
      const scheme = await tx.assessmentScheme.findUniqueOrThrow({
        where: { id: schemeId },
      });
      if (scheme.status !== "DRAFT")
        throw new ConflictException(
          "Only draft assessment schemes can be configured",
        );
      ...
      await tx.auditLog.create({
        data: {
          ...
          targetTable: "assessment_schemes",
          targetId: schemeId,
          newValues: { componentCount: rows.length, totalWeight: total },
          actorId,
        },
      });
  ...
  async finalizeScheme(
    ...
    if (!scheme)
      throw new BadRequestException("No draft assessment scheme exists");
    await this.authorization.assertIndependentApproval(
      scheme.createdById,
      actorId,
      "assessment scheme",
    );
    const total = scheme.components.reduce((n, c) => n + Number(c.weight), 0);
    if (Math.abs(total - 100) > 0.001 || !scheme.components.length)
      throw new BadRequestException(
        "Assessment scheme must contain components totalling exactly 100%",
      );
    return this.prisma.assessmentScheme.update({
      where: { id: scheme.id },
      data: {
        status: "ACTIVE",
  ...
  async saveMark(
    ...
    if (component.scheme.courseOfferingId !== dto.courseOfferingId)
      throw new BadRequestException(
        "Assessment component does not belong to this course offering",
      );
    if (component.scheme.status !== "ACTIVE")
      throw new ConflictException("Assessment scheme is not active");
    if (dto.score > Number(component.maxScore))
      throw new BadRequestException(
        `Score cannot exceed ${component.maxScore}`,
      );
    ...
    const existing = await this.prisma.assessmentMark.findUnique({
      where: {
        uq_assessment_mark_student_component: {
          studentId: dto.studentId,
    ...
    if (existing?.status === "FINALIZED")
      throw new ConflictException(
        "Finalized assessment marks require a controlled amendment workflow.",
      );
  ...
  async getGradebook(
    ...
    const offering = await this.prisma.courseOffering.findUnique({
      ...
      select: {
        ...
        semesterId: true,
        course: { select: { code: true, title: true } },
        semesterModel: { select: { name: true } },
      },
    });
    if (!offering)
      throw new BadRequestException("Course offering was not found");
    const scheme = await this.prisma.assessmentScheme.findFirst({
    ...
    const required = scheme.components.filter(
      (component) => component.isRequired,
    );
    const search = pagination?.search?.trim();
    const registrationWhere: Prisma.CourseRegistrationWhereInput = {
      courseOfferingId,
      status: { in: ["REGISTERED", "COMPLETED"] },
      ...(search
        ? {
    ...
    const [total, registrations, summaryRegistrations] = isPaged
      ? await Promise.all([
          this.prisma.courseRegistration.count({ where: registrationWhere }),
    ...
    const makeRow = (registration: (typeof registrations)[number]) => {
      ...
      const finalized =
        complete &&
        required.every(
          (component) => markMap.get(component.id)?.status === "FINALIZED",
        );
      return {
    ...
    const rows = registrations.map(makeRow);
    const summaryStudentIds = new Set(
      (summaryRegistrations ?? registrations).map(
    ...
    for (const studentId of summaryStudentIds) {
      ...
      const isComplete = required.every((component) =>
        markMap.has(component.id),
      );
      if (isComplete) {
        complete += 1;
        if (
    ...
    const summaryTotal = total ?? registrations.length;
    return {
      offering,
      scheme,
      rows,
      ...
      ...(isPaged
        ? {
            pagination: {
              ...
              pageSize,
              total: summaryTotal,
              totalPages: Math.max(1, Math.ceil(summaryTotal / pageSize)),
            },
          }
        : {}),
    };
  ...
  async exportGradebook(
    ...
    const headers = [
      ...
      "Matric No",
      "Name",
      ...gb.scheme.components.map((c) => c.code),
      "Final Score",
      "Grade",
      "Grade Point",
    ];
    ...
    for (const r of gb.rows) {
      const vals = gb.scheme.components.map((c) => {
        const m = r.marks.find((x) => x.componentId === c.id);
        return m?.score ?? "";
      });
      const gr = r.complete
        ? computeGradeForSystem(r.finalScore, system)
        : { grade: "", gradePoint: "" };
      lines.push(
        [
          r.student.id,
  ...
  async generateDraftResults(
    courseOfferingId: string,
    actorId: string,
    actorRole: AcademicActorRole = "STAFF",
  ) {
    const gb = await this.getGradebook(courseOfferingId, actorId, actorRole);
    const selfEnteredBy = gb.rows
      .flatMap((row) => row.marks as Array<{ enteredById?: string }>)
      .find((mark) => mark.enteredById === actorId)?.enteredById;
    if (selfEnteredBy) {
      await this.authorization.assertIndependentApproval(
        selfEnteredBy,
        actorId,
    ...
    if (gb.summary.incomplete)
      throw new BadRequestException(
        `${gb.summary.incomplete} student(s) have incomplete assessment marks`,
      );
    if (gb.summary.unfinalized)
      throw new ConflictException(
        `${gb.summary.unfinalized} student(s) have unfinalized assessment marks`,
      );
    const settings = await this.prisma.institutionSettings.findFirst({
      select: { gradingSystem: true, gradePolicyVersion: true },
    });
    const system = (settings?.gradingSystem ?? "NIGERIAN_5_POINT") as any;
    const offering = await this.prisma.courseOffering.findUniqueOrThrow({
      where: { id: courseOfferingId },
      include: {
        course: { select: { creditUnits: true, code: true } },
        semesterModel: { select: { id: true } },
      },
    });
    if (!offering.semesterId)
      throw new BadRequestException(
        "Course offering is not linked to a semester",
      );
    const studentIds = gb.rows.map((row) => row.student.id);
    const absentAttendance =
      studentIds.length === 0
        ? []
        : await this.prisma.examAttendance.findMany({
            where: {
    ...
    const absentStudentIds = new Set(
      absentAttendance.map((record) => record.studentId),
    );
    return this.prisma.$transaction(async (tx) => {
      const studentIds = gb.rows.map((row) => row.student.id);
      const [existingResults, priorResults] = await Promise.all([
        ...
        tx.studentResult.findMany({
          where: {
            studentId: { in: studentIds },
            courseOffering: { courseId: offering.courseId },
          },
      ...
      for (const result of priorResults) {
        if (!priorAttemptByStudent.has(result.studentId))
          priorAttemptByStudent.set(result.studentId, result.attemptNumber);
      }
      ...
      for (const row of gb.rows) {
        ...
        const calc = computeGradeForSystem(
          effectiveFinalScore,
          system,
          absentFromExam,
        );
        ...
        const prior =
          ...
          (priorAttemptByStudent.get(row.student.id) ?? 0) + 1;
        const assessmentEvidence = {
          ...
          components: gb.scheme.components.map((component) => {
            ...
            return {
              ...
              markVersion: mark?.version ?? null,
              examTimetableId: mark?.examTimetableId ?? null,
            };
          }),
        ...
        writes.push({
          ...
          data: {
            ...
            gradingSystemSnapshot: system,
            gradingPolicyVersion: settings?.gradePolicyVersion ?? 1,
            assessmentEvidence,
            attemptNumber: prior,
            absentFromExam,
            status: "DRAFT" as const,
            submittedById: actorId,
            approvedByHodId: null,
            hodApprovedAt: null,
            approvedByDeanId: null,
            deanApprovedAt: null,
            senatePendingAt: null,
            senatePublishedAt: null,
      ...
      return {
        ...
        skipped: gb.rows.length - out.length,
  ...
  async uploadCsv(
    ...
    if (!offering.semesterId || offering.semesterId !== dto.semesterId)
      throw new BadRequestException(
        "Semester does not match the course offering",
      );
    const scheme = await this.prisma.assessmentScheme.findFirst({
    ...
    if (studentIdIndex === undefined && matricNoIndex === undefined)
      throw new BadRequestException("CSV must contain Student ID or Matric No");
    ...
    if (missingComponents.length)
      throw new BadRequestException(
        `CSV is missing component columns: ${missingComponents.join(", ")}`,
      );
    ...
    const byMatricNo = new Map(
      registrations.map((registration) => [
        registration.student.matricNo.toLowerCase(),
        registration.student.id,
      ]),
    );
    const errors: Array<{
    ...
    for (let rowIndex = 1; rowIndex < records.length; rowIndex += 1) {
      ...
      const suppliedStudentId =
        studentIdIndex === undefined
          ? ""
          : (values[studentIdIndex]?.trim() ?? "");
      const suppliedMatricNo =
        matricNoIndex === undefined
          ? ""
          : (values[matricNoIndex]?.trim() ?? "");
      const studentId = suppliedStudentId
        ? byStudentId.get(suppliedStudentId)
        : byMatricNo.get(suppliedMatricNo.toLowerCase());
      const matricStudentId = suppliedMatricNo
        ? byMatricNo.get(suppliedMatricNo.toLowerCase())
        : undefined;
      const rowErrors: string[] = [];
      if (!studentId)
        rowErrors.push("Student is not registered for this course offering");
      if (studentId && suppliedMatricNo && matricStudentId !== studentId)
        rowErrors.push("Student ID and Matric No do not match");
      if (studentId && seenStudents.has(studentId))
        rowErrors.push("Duplicate student row");
      ...
      for (const component of scheme.components) {
        ...
        if (
          ...
          rowErrors.push(
            `${component.code} must be between 0 and ${component.maxScore}`,
          );
        else if (studentId)
          parsedMarks.push({
            row: rowIndex + 1,
      ...
      if (rowErrors.length)
        errors.push({
          row: rowIndex + 1,
          studentId,
          matricNo: suppliedMatricNo || undefined,
    ...
    const existingMarks = await this.prisma.assessmentMark.findMany({
      where: {
        courseOfferingId: dto.courseOfferingId,
        studentId: { in: [...seenStudents] },
        componentId: { in: scheme.components.map((component) => component.id) },
      },
      select: { studentId: true, componentId: true, status: true },
    });
    const finalizedKeys = new Set(
      existingMarks
        .filter((mark) => mark.status === "FINALIZED")
        .map((mark) => `${mark.studentId}:${mark.componentId}`),
    );
    const finalizedErrors = parsedMarks
      .filter((mark) =>
        finalizedKeys.has(`${mark.studentId}:${mark.componentId}`),
      )
      .map((mark) => ({
        row: mark.row,
        studentId: mark.studentId,
        error: "Finalized marks require a controlled amendment workflow",
      }));
    if (finalizedErrors.length) {
      await this.prisma.gradeUploadBatch.update({
        where: { id: batch.id },
        data: {
          status: "REJECTED",
          errorRows: finalizedErrors.length,
          errorReport: finalizedErrors,
        },
      });
      return {
        batchId: batch.id,
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        for (let offset = 0; offset < parsedMarks.length; offset += 100) {
          ...
          if (Number(affected) !== chunk.length)
            throw new ConflictException(
              "A finalized mark changed while this upload was applying. Validate the file again before retrying.",
            );
          appliedMarks += Number(affected);
        }
        await tx.gradeUploadBatch.update({
          where: { id: batch.id },
          data: {
            status: "APPLIED",
            validRows: validStudentRows,
            errorRows: 0,
          },
        });
        await tx.auditLog.create({
          data: {
            action: AuditAction.UPDATE,
            targetTable: "grade_upload_batches",
            targetId: batch.id,
            newValues: {
              courseOfferingId: dto.courseOfferingId,
              checksum,
      ...
      await this.prisma.gradeUploadBatch
        .update({
          where: { id: batch.id },
          data: {
            status: "FAILED",
  ...
  async finalizeMarks(
    ...
    const gradebook = await this.getGradebook(
      courseOfferingId,
      actorId,
      actorRole,
    );
    if (gradebook.summary.incomplete)
      throw new BadRequestException(
        `${gradebook.summary.incomplete} student(s) have incomplete assessment marks`,
      );
    ...
    await this.audit.log(
      {
        action: AuditAction.UPDATE,
        targetTable: "assessment_marks",
        targetId: courseOfferingId,
        newValues: { status: "FINALIZED", count: result.count },
      },
      actorId,
    );
