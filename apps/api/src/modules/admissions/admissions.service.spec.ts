...
const TEST_TRACKING_SECRET =
  "test-admissions-tracking-secret-012345678901234567890123456789";
const TEST_PHOTO_PROOF = (() => {
  const payload = Buffer.from(
    JSON.stringify({
      idempotencyKey: TEST_IDEMPOTENCY,
      key: `admissions/pre-submit/${TEST_IDEMPOTENCY}/passport-photo/test.jpg`,
  ...
  return `${payload}.${createHmac("sha256", TEST_TRACKING_SECRET).update(payload).digest("hex")}`;
})();
const makeCycle = (o: Partial<Record<string, unknown>> = {}) => ({
  ...
  isActive: true,
  openDate: new Date(Date.now() - 180 * DAY),
  closeDate: new Date(Date.now() + 180 * DAY),
  utmeMinScore: 180,
  maxApplicants: null,
  ...o,
});
...
const makeApplicant = (o: Partial<Record<string, unknown>> = {}) => ({
  id: "app-1",
  applicationNo: "20250UTME00001",
  firstName: "Adewale",
  lastName: "Ojo",
  email: "adewale@test.com",
  phone: "08012345678",
  dateOfBirth: new Date("2003-06-15"),
  gender: "Male",
  nationality: "Nigerian",
  admissionType: AdmissionType.UTME,
  admissionCycleId: "cycle-1",
  programmeChoice1Id: "prog-1",
  programmeChoice2Id: null,
  programmeChoice3Id: null,
  jambRegNo: "12345678AB",
  jambScore: 220,
  jambVerified: true,
  status: ApplicantStatus.PENDING,
  student: null,
  admissionCycle: makeCycle(),
  application: null,
  deletedAt: null,
  createdAt: new Date(),
  updatedAt: new Date(),
...
describe("AdmissionsService", () => {
  ...
  let directOLevelSitting: { create: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    prisma = {
      ...
      oLevelSitting: {
        create: jest.fn().mockResolvedValue({ id: "sitting-1" }),
        deleteMany: jest.fn(),
      },
      ...
      programme: {
        findUniqueOrThrow: jest
          .fn()
          .mockResolvedValue({ id: "prog-1", name: "BSc CS" }),
    ...
    const direct = {
      $transaction: jest.fn((fn: (tx: unknown) => unknown) =>
        fn({
          ...
          previousEducation: { createMany: jest.fn() },
          oLevelSitting: directOLevelSitting,
          oLevelSubject: { createMany: jest.fn() },
          applicationDocument: {
            create: jest
              .fn()
              .mockResolvedValue({ id: "document-1", status: "PENDING" }),
          },
  ...
  describe("createCycle()", () => {
    ...
    it("rejects when close date is before open date", async () => {
      ...
      ).rejects.toThrow("Close date must be after open date");
    });
  });
  ...
  describe("trackPublicApplication()", () => {
    it("returns status only with the valid issued tracking credential", async () => {
      ...
      const token = crypto
        .createHmac("sha256", process.env.ADMISSIONS_TRACKING_SECRET!)
        .update("20250UTME00001:adewale@test.com")
        .digest("hex");
      const result = await svc.trackPublicApplication({
        applicationNo: "20250UTME00001",
        trackingToken: token,
      });
    ...
    it("uses a generic not-found response for an invalid credential", async () => {
      ...
      await expect(
        svc.trackPublicApplication({
          applicationNo: "20250UTME00001",
          trackingToken: "0".repeat(64),
        }),
  ...
  describe("apply()", () => {
    const validDto = {
      ...
      passportPhotoProof: TEST_PHOTO_PROOF,
    };
    ...
    it("encrypts NIN at rest and excludes it from the public response", async () => {
      ...
      const result = await svc.apply(
        { ...validDto, nin: rawNin, ninConsentAccepted: true },
        TEST_IDEMPOTENCY,
      );
      const storedNin = prisma.applicant.create.mock.calls[0]?.[0]?.data
        ?.nin as string;
    ...
    it("rejects NIN submission without the privacy consent acknowledgment", async () => {
      await expect(
        svc.apply({ ...validDto, nin: "12345678901" }, TEST_IDEMPOTENCY),
    ...
    it("rejects a client admission type that conflicts with the selected cycle", async () => {
      await expect(
        svc.apply(
          { ...validDto, admissionType: "DE" as never },
          TEST_IDEMPOTENCY,
        ),
    ...
    it("rejects applicant under 16 years old", async () => {
      ...
      await expect(
        svc.apply(
          { ...validDto, dateOfBirth: youngDob.toISOString().split("T")[0]! },
          TEST_IDEMPOTENCY,
        ),
    ...
    it("rejects when cycle is not active", async () => {
      prisma.admissionCycle.findUniqueOrThrow.mockResolvedValue(
        makeCycle({ isActive: false }),
      );
      await expect(svc.apply(validDto, TEST_IDEMPOTENCY)).rejects.toThrow(
        UnprocessableEntityException,
      );
    });
    ...
    it("rejects when programme choices are the same", async () => {
      await expect(
        svc.apply(
          { ...validDto, programmeChoice2Id: "prog-1" },
          TEST_IDEMPOTENCY,
        ),
    ...
    it("persists sponsor data in admissionDetails and identifiers on the sitting", async () => {
      await svc.apply(
        {
          ...validDto,
          guardian: {
          ...
          sponsor: { sponsorType: "PARENT_GUARDIAN", sameAsGuardian: true },
          oLevelResults: [
            {
              subject: "English Language",
              grade: "C6",
              examType: "WAEC" as never,
              examYear: 2025,
              sittingNumber: 1,
              candidateNumber: "NG123456",
              examinationNumber: "WAE20251234",
              centreNumber: "12345",
            },
            {
              subject: "Mathematics",
      ...
      const applicantCreate = prisma.applicant.create.mock.calls.at(-1)?.[0];
      expect(applicantCreate?.data?.admissionDetails).toMatchObject({
        sponsor: expect.objectContaining({
          sponsorType: "PARENT_GUARDIAN",
          sameAsGuardian: true,
          fullName: "Amina Ojo",
          phone: "08012345678",
        }),
      });
      expect(directOLevelSitting.create).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it("rejects conflicting sitting-level identifiers instead of silently corrupting a certificate record", async () => {
      await expect(
        svc.apply(
          {
            ...validDto,
            oLevelResults: [
              ...
              },
              {
                ...
                examYear: 2025,
                sittingNumber: 1,
                candidateNumber: "SECOND",
              },
            ],
          } as never,
          TEST_IDEMPOTENCY,
        ),
      ).rejects.toThrow("same examination metadata");
    });
  });
  ...
  describe("updateStatus()", () => {
    it("transitions PENDING → SCREENED", async () => {
      prisma.applicant.update.mockResolvedValue(
        makeApplicant({ status: ApplicantStatus.SCREENED }),
      );
    ...
    it("rejects invalid FSM transition (PENDING → OFFERED)", async () => {
      await expect(
        svc.updateStatus("app-1", { status: "OFFERED" }, "actor"),
    ...
    it("requires rejection reason on REJECTED status", async () => {
      ...
      await expect(
        svc.updateStatus("app-1", { status: "REJECTED" }, "actor"),
    ...
    it("allows SCREENED → REJECTED with reason provided", async () => {
      ...
      prisma.applicant.update.mockResolvedValue(
        makeApplicant({ status: ApplicantStatus.REJECTED }),
      );
      const result = await svc.updateStatus(
        "app-1",
        {
          status: "REJECTED",
          rejectionReason: "Insufficient UTME score for programme",
        },
        "actor",
      );
  ...
  describe("screenBulk()", () => {
    it("dry-run reports counts without saving", async () => {
      ...
      const result = await svc.screenBulk(
        { admissionCycleId: "cycle-1", dryRun: true },
        "actor",
      );
    ...
    it("applies screening when dryRun=false", async () => {
      ...
      const result = await svc.screenBulk(
        { admissionCycleId: "cycle-1", dryRun: false },
        "actor",
      );
  ...
  describe("recordOLevelResults() append mode", () => {
    it("merges a newly recorded subject instead of deleting prior evidence", async () => {
        ...
        oLevelSittings: [
          {
            sittingNumber: 1,
            examType: "WAEC",
            examinationAuthorityId: null,
            examinationTypeId: null,
            candidateCategory: null,
            examYear: 2025,
            candidateNumber: "CAND-1",
            examinationNumber: "EXAM-1",
            centreNumber: "CENTRE-1",
            subjects: [
              { subjectId: null, subject: "English Language", grade: "C6" },
            ],
          },
        ],
      });
      const txOLevelSitting = {
        deleteMany: jest.fn(),
        create: jest.fn().mockResolvedValue({ id: "new-sitting" }),
      };
      const txOLevelSubject = { createMany: jest.fn() };
      const txApplication = {
      ...
      prisma.$transaction.mockImplementationOnce(
        (fn: (tx: unknown) => unknown) =>
          fn({
            applicant: { update: jest.fn().mockResolvedValue(makeApplicant()) },
            application: txApplication,
            oLevelSitting: txOLevelSitting,
            oLevelSubject: txOLevelSubject,
          }),
      );

      await svc.recordOLevelResults(
        "app-1",
        {
      ...
      expect(txOLevelSubject.createMany).toHaveBeenCalledWith({
        data: expect.arrayContaining([
          expect.objectContaining({ subject: "English Language", grade: "C6" }),
          expect.objectContaining({ subject: "Mathematics", grade: "C5" }),
        ]),
      });
      expect(txOLevelSitting.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            candidateNumber: "CAND-1",
            examinationNumber: "EXAM-1",
  ...
  describe("configured O'Level policy evaluation", () => {
    const evaluate = (results: unknown[], policy?: unknown) =>
      (
        svc as unknown as {
          evaluateOLevelEligibility: (rows: unknown[], p?: unknown) => unknown;
        }
    ...
    it("honors configured credit, sitting, language, and programme-subject rules", () => {
      ...
      expect(result.reasons).toContain(
        "Required O'Level subject credit missing: Biology (Agricultural Science).",
      );
    });
  });
});
