...
export type AssessmentGradebook = {
  offering?: { id: string; semesterId: string | null; course: { code: string; title: string }; semesterModel?: { name: string } | null };
  ...
  rows: Array<{ student: { id: string; matricNo: string; firstName: string; lastName: string }; marks: Array<{ componentId: string; score: number; status?: string }>; finalScore: number; complete: boolean; finalized?: boolean }>;
  summary: { total: number; complete: number; incomplete: number; finalized?: number; unfinalized?: number };
};
...
export type GradeUploadResult = {
  ...
  errorRows: number;
  appliedMarks: number;
  errors: Array<{ row: number; studentId?: string; matricNo?: string; error: string }>;
};
...
export function useFinalizeAssessmentMarks() {
  return useMutation({
    mutationFn: (courseOfferingId: string) => apiClient.post<{ courseOfferingId: string; finalized: number; finalizedAt: string }>(`/assessment/offerings/${courseOfferingId}/finalize-marks`),
  });
}
...
export function useAssessmentTemplate() {
  return useMutation({ mutationFn: (courseOfferingId: string) => apiClient.download(`/assessment/offerings/${courseOfferingId}/template`) });
}

export function useAssessmentCsvUpload() {
  return useMutation({
    mutationFn: (input: { courseOfferingId: string; semesterId: string; csv: string; fileName?: string; mode: 'VALIDATE_ONLY' | 'APPLY' }) =>
