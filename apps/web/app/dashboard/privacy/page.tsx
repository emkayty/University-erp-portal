...
export default function PrivacyPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle>Rectification and restriction</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2"><Button variant="outline" disabled={!selfOrDpo || busy || (!email && !phone)} onClick={() => act(() => apiClient.post(`/privacy/rectify/${subjectId}`, { email: email || undefined, phone: phone || undefined, reason: reason || undefined }), 'Rectification completed and audited.')}>Apply rectification</Button><Button variant="outline" disabled={role !== 'SUPER_ADMIN' && !(role === 'STAFF')} onClick={() => act(() => apiClient.post(`/privacy/restrict/${subjectId}`, { reason: reason || 'Operational privacy restriction requested' }), 'Processing restriction recorded.')}>Restrict processing</Button></CardContent></Card>
    </div>
    {role === 'SUPER_ADMIN' && <Card className="border-red-200"><CardHeader><CardTitle className="text-red-800">Controlled erasure</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-muted-foreground">Erasure requires a distinct active VC approval reference. Academic legal holds result in pseudonymisation rather than hard deletion.</p><Input value={vcApprovalReference} onChange={(e) => setVcApprovalReference(e.target.value)} placeholder="Active VC user UUID" /><Button variant="destructive" disabled={busy || !vcApprovalReference || !reason || !confirmReady(subjectId)} onClick={() => { if (window.confirm('Confirm this irreversible privacy erasure request?')) void act(() => apiClient.delete(`/privacy/erase/${subjectId}`, { vcApprovalReference, reason }), 'Erasure completed and compliance evidence recorded.'); }}>Request erasure</Button></CardContent></Card>}
  </div>;
}
