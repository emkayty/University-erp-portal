...
export default function SmartOperationsPage() {
  ...
  const {
    ...
    isError: tasksError,
  } = useIntelligenceTasks(status || undefined);
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
          Review prioritized signals, claim work, record decisions, and close
          the operational loop. Automation proposes actions; authorized staff
          remain accountable for consequential decisions.
        </p>
      </header>
      {(alertsError || tasksError) && (
        <div
          role="alert"
          className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800"
        >
          Unable to load the latest operational queue. Try again shortly.
        </div>
      )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <CardTitle className="text-base">
                Human-review tasks ({tasks.length})
              </CardTitle>
              <select
                ...
                <article key={task.id} className="rounded-lg border p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {task.description ||
                          "Review the item and record the appropriate institutional action."}
                      </p>
                    </div>
                        ...
                        <>
                          <label className="text-xs text-muted-foreground">
                            ...
                            <input
                              ...
                              value={assigneeByTask[task.id] ?? ""}
                              onChange={(event) =>
                                setAssigneeByTask((current) => ({
                                  ...current,
                          ...
                          <Button
                            ...
                            disabled={
                              ...
                              !(assigneeByTask[task.id] ?? "").trim()
                            }
                            onClick={() =>
                              assignTask.mutate({
                                id: task.id,
                                assigneeId: assigneeByTask[task.id].trim(),
                              })
                            }
                          >
                            Assign
