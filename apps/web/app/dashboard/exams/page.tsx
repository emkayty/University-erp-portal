...
export default function ExamsPage() {
  ...
  return <div className="space-y-6">
    ...
    {activeSemester && <Card><CardHeader><CardTitle>Official timetable</CardTitle></CardHeader><CardContent className="space-y-3">{timetable.isLoading ? <p className="text-sm text-muted-foreground">Loading timetable…</p> : timetable.data?.length ? timetable.data.map((exam) => <article key={exam.id} className={`rounded-lg border p-4 ${selectedExam === exam.id ? 'border-[--color-primary]' : ''}`}><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="font-medium">{exam.courseOffering?.course?.code ?? 'Course'} · {exam.courseOffering?.course?.title ?? 'Examination'}</h2><p className="text-sm text-muted-foreground">{new Date(exam.examDate).toLocaleDateString()} · {exam.startTime} · {exam.durationMinutes} minutes · {exam.venue}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setSelectedExam(exam.id)}>View report</Button>{operator && <Button size="sm" onClick={() => generate.mutate(exam.id)} loading={generate.isPending}>Generate candidates</Button>}{author && <><Button size="sm" variant="outline" onClick={() => beginEdit(exam)}>Reschedule</Button><Button size="sm" variant="destructive" onClick={() => cancelExam(exam.id)} loading={cancelTimetable.isPending}>Cancel exam</Button></>}</div></div></article>) : <p className="py-6 text-sm text-muted-foreground">No timetable entries found.</p>}</CardContent></Card>}
