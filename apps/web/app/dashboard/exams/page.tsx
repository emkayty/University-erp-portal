...
import { useSemesters } from '@/hooks/use-results';
...
export default function ExamsPage() {
  ...
  const candidates = useQuery({ queryKey: ['exams', 'candidates', selectedExam], queryFn: () => apiClient.get<Array<{ studentId: string; eligibility: string; reason?: string; student?: { matricNo: string; firstName: string; lastName: string } }>>(`/exams/timetable/${selectedExam}/candidates`), enabled: Boolean(selectedExam) });
  ...
  return <div className="erp-workspace-page">
    <header className="erp-workspace-header"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[--color-primary]">Examination operations</p><h1 className="text-2xl font-semibold">Exams Workspace</h1><p className="mt-1 text-sm text-muted-foreground">Author and safely reschedule official examinations, generate eligible candidates, and monitor attendance coverage before results processing.</p></header>
    <Card className="erp-control-rail"><CardContent className="pt-5"><form onSubmit={(e) => { e.preventDefault(); setActiveSemester(semesterId); setSelectedExam(''); setEditingExam(null); }} className="flex flex-col gap-3 sm:flex-row"><label htmlFor="exams-semester" className="sr-only">Semester</label><select id="exams-semester" value={semesterId} onChange={(e) => setSemesterId(e.target.value)} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm sm:w-auto" required><option value="">Choose semester</option>{semesters.map((semester) => <option key={semester.id} value={semester.id}>{semester.name}</option>)}</select><Button type="submit">Load timetable</Button></form></CardContent></Card>
    ...
    {selectedExam && <div className="grid gap-4 md:grid-cols-2"><Card><CardHeader><CardTitle>Attendance report</CardTitle></CardHeader><CardContent>{report.isLoading ? <p className="text-sm text-muted-foreground">Loading report…</p> : report.data ? <div className="grid grid-cols-2 gap-3 text-sm"><Metric label="Candidates" value={report.data.candidates.total} /><Metric label="Eligible" value={report.data.candidates.eligible} /><Metric label="Attendance records" value={report.data.attendance.total} /><Metric label="Present" value={report.data.attendance.present ?? 0} /><Metric label="Coverage" value={report.data.attendance.attendancePct ?? 0} /><Metric label="Missing" value={report.data.attendance.missing} /></div> : <p className="text-sm text-muted-foreground">No report available.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Candidate list</CardTitle></CardHeader><CardContent className="space-y-2">{candidates.isLoading ? <p className="text-sm text-muted-foreground">Loading candidates…</p> : candidates.data?.length ? candidates.data.map((candidate) => <div key={candidate.studentId} className="flex flex-col gap-2 rounded border p-2 text-sm sm:flex-row sm:items-center sm:justify-between"><span className="text-sm font-medium">{candidate.student ? `${candidate.student.matricNo} · ${candidate.student.firstName} ${candidate.student.lastName}` : 'Candidate record'}</span><span>{candidate.eligibility}</span>{operator && candidate.eligibility === 'ELIGIBLE' && <div className="flex gap-1"><Button size="sm" variant="outline" onClick={() => attendance.mutate({ studentId: candidate.studentId, status: 'PRESENT' })} loading={attendance.isPending}>Present</Button><Button size="sm" variant="outline" onClick={() => attendance.mutate({ studentId: candidate.studentId, status: 'ABSENT' })} loading={attendance.isPending}>Absent</Button></div>}</div>) : <p className="text-sm text-muted-foreground">Candidates have not been generated.</p>}</CardContent></Card></div>}
