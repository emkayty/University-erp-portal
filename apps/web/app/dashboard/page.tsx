  ...
  return <div className="mx-auto w-full max-w-[1440px] space-y-5 pb-10">
    <header className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl"><div className="flex flex-wrap items-center gap-2 text-xs font-medium text-[--color-primary]"><span>{salutation}, {displayName}</span><span className="rounded-full bg-[--color-primary]/10 px-2 py-1">{ROLE_LABELS[role]}</span></div><h1 className="mt-2 text-2xl font-bold tracking-tight sm:text-4xl">{copy.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{copy.description}</p></div>
    ...
    {!hidden.actions ? <section aria-labelledby="actions-heading"><div className="mb-3 flex items-end justify-between gap-4"><div><h2 id="actions-heading" className="text-base font-semibold">Next best actions</h2><p className="text-sm text-muted-foreground">Clear entry points for the work most relevant to your role.</p></div><Bell className="h-5 w-5 text-muted-foreground" /></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{quickActions.map((action) => { const Icon = action.icon; return <Link key={action.href} href={action.href} className="group rounded-2xl border border-border bg-card p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[--color-primary]/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[--color-primary]"><div className="flex items-start justify-between gap-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[--color-primary]/10 text-[--color-primary]"><Icon className="h-5 w-5" /></span><ArrowRight className="h-4 w-4 text-muted-foreground transition group-hover:translate-x-0.5 group-hover:text-[--color-primary]" /></div><h3 className="mt-4 text-sm font-semibold">{action.label}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{action.description}</p></Link>; })}</div></section> : null}
